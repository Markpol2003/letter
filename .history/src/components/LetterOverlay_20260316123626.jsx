...
/* ══════════════════════════════════════════
   SMOOTH SCROLL HOOK
   Uses requestAnimationFrame + easing so
   the wheel scrolls feel buttery smooth.
══════════════════════════════════════════ */
function useSmoothScroll(ref) {
  useEffect(() => {
    ...
    /* ease-out factor — lower = smoother/slower */
    const EASE = 0.09;
...
export default function LetterOverlay({ onClose }) {
  ...
  /* attach smooth scroll to the content div */
  ...
  return (
    <div className={styles.overlay} ref={overlayRef} onClick={handleBackdrop}>
      <div className={styles.letter}>
        ...
        {/* attach ref here for smooth scroll */}
